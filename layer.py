...
if conn:
    ...
    list=[]
    duplicate = 0
    layerColumnName=('factor_id','near_id', 'color', 'level')
    list.append(layerColumnName)
    sql = "SELECT `from_id`, `to_id` FROM `trans`.`relationship` where `from_id`=%d"%(factorId) # 找和起始點相關的第一層節點
    cursor.execute(sql)# 執行 SQL 
    layer1Result = cursor.fetchall()# 取出全部資料
    print(layer1Result)
    if layer1Result!=None:
        from1_id=layer1Result[1][0]
        layerValues=(from1_id,from1_id,'#ffcc33','起始點')
        list.append(layerValues)
        for i in range(1,len(layer1Result)): 
            from1_id=layer1Result[i][0]
            to1_id=layer1Result[i][1]
           # weight1=layer1Result[i][2]
            # layerValues=(from1_id,from1_id,'#ffcc33','起始點')
            # list.append(layerValues)
            layer1Values=(from1_id,to1_id,'#699c4c','第1層')
            list.append(layer1Values)
            sql = "SELECT `from_id`, `to_id` FROM `trans`.`relationship` where `from_id`=%d"%(to1_id)
            cursor.execute(sql)# 執行 SQL 
           ...
           # print(layer2Result)
            for j in range(len(layer2Result)):
               ...
               # weight2=layer2Result[j][2]
                # layer2Values = (from2_id,factorId,'#699c4c','第1層')
                # if layer2Values not in list:
                layer2Values = (from2_id,to2_id,'#0066cc','第2層')
        ...
        # conn.commit()# 提交到 SQL
        np.savetxt("numpy_test.csv", list, delimiter =",",fmt ='% s')
cursor.close()
