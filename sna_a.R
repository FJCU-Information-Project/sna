...
node_data_frame<<-data.frame(node=numeric(n),first=numeric(n),second=numeric(n),third=numeric(n),forth=numeric(n),fifth=numeric(n))
relationship_data_frame<<-data.frame(first=numeric(n),second=numeric(n),third=numeric(n),forth=numeric(n),fifth=numeric(n))
count <<- 1
  ...
  testbind<-rbind(node_data_frame$node,bb$id[1])
...
# {
  sna_a<-visNetwork(node_list,relationship_list, width = "100%", height = "500px")%>%
    visNodes(size = 30)%>%
