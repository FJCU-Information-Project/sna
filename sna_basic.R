...
#計算degree_centrality
i <- 1
degree_centrality_table <- data.frame(id=c(1:count_node),degree=c(1:count_node))
while (i <= count_node) {
  ...
  degree_centrality_table[i,2] <- data.frame(degree=c(every_degree_centrality))
  i <- i + 1
}
...
j <- 1
closeness_centrality_table <- data.frame(id=c(1:count_node),closeness=(1:count_node))
while (j <= count_node) {
  ...
  closeness_centrality_table[j,2] <- data.frame(closeness=c(every_closeness_centrality))
  j <- j + 1
}
#closeness_centrality_table <- subset(closeness_centrality_table,closeness != "NA" & closeness != 0)

basic_table <- data.frame(id=c(degree_centrality_table$id),degree_centrality=c(degree_centrality_table$degree),closeness_centrality=c(closeness_centrality_table$closeness))
...
node <- data.frame(id=c(inId_is_from_id$to_id,inId_is_to_id$from_id),label = c(inId_is_from_id$name,inId_is_to_id$name),title = c(inId_is_from_id$name,inId_is_to_id$name),font.size = 20)
edge <- data.frame(from=c(inId), to=c(node$id), value=c(inId_is_from_id$total,inId_is_to_id$total),width=c(inId_is_from_id$total,inId_is_to_id$total))
