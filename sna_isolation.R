...
print("21")
print(inId)
select_isolation <- data.frame(c(dbGetQuery(connect ,paste("SELECT * FROM trans.default_relationship where from_id = ",inId,"or to_id = ",inId," and total = 0"))))
select_node <- data.frame(c(dbGetQuery(connect ,paste("SELECT * FROM trans.default_relationship where from_id = ",inId,"or to_id = ",inId))))
select_draw_edge <- data.frame(c(dbGetQuery(connect ,paste("SELECT * FROM trans.default_relationship where from_id = ",inId,"or to_id = ",inId," and total != 0"))))
print(select_isolation)
print(select_node)
from_id_name <- data.frame(c(dbGetQuery(connect ,paste("select from_id,`name` from trans.node n,trans.default_relationship d where from_id = ",inId,"or to_id = ",inId," and n.id = d.from_id"))))
to_id_name <- data.frame(c(dbGetQuery(connect ,paste("select to_id,`name` from trans.node n,trans.default_relationship d where from_id = ",inId,"or to_id = ",inId," and n.id = d.to_id"))))
table_from_id_name <- data.frame(c(dbGetQuery(connect ,paste("select from_id,`name` from trans.node n,trans.default_relationship d where from_id = ",inId,"or to_id = ",inId," and total = 0 and n.id = d.from_id"))))
table_to_id_name <- data.frame(c(dbGetQuery(connect ,paste("select to_id,`name` from trans.node n,trans.default_relationship d where from_id = ",inId,"or to_id = ",inId," and total = 0 and n.id = d.to_id"))))
print("25")
