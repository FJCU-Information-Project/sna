...
layerUserId <- args[1] #(資料庫名稱，使用者名稱user_id)
layerDatasetId <- args[2] #(資料表代號，dataset)
...
if(nrow(layer_csv) != 0){
  ...
  layer_csv<-layer_csv[order(-layer_csv$weight),]
  ...
  nodes <- data.frame(id = 1,label = paste("original factor"))
  edges <- data.frame(from = c(1,1), to = c(1,1))
  draw_sna_layer<-visNetwork(nodes,edges, width = "100%", height = "500px")%>%
    visNodes(size = 30)%>%
    visOptions(highlightNearest = TRUE
              ,nodesIdSelection = list(enabled = TRUE
                                        ,style = 'width: 200px; height: 26px;
  ...
  visSave(draw_sna_layer, file = paste0("..",.Platform$file.sep,"Flask",.Platform$file.sep,"templates",.Platform$file.sep,"layer.html"), selfcontained = FALSE)
  write.csv(layer_csv,"../flask/layerHtmlTable.csv", row.names = FALSE, fileEncoding = "UTF-8")
